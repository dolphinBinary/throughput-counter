...
double calculateThroughput(const std::vector<Stats>& stats)
{
    ...
    //TODO: Возвращать ошибку отрицательного времени
...
{
    std::vector<Stats> dlStats = readStats(argv[1]);
    std::vector<Stats> ulStats = readStats(argv[2]);

    double dlThroughput = calculateThroughput(dlStats);
    double ulThroughput = calculateThroughput(ulStats);

    std::cout << "Передающая пропускная способность: " << dlThroughput / 1000 << " Кбит/с" << std::endl;
    std::cout << "Принимающая пропускная способность: " << ulThroughput / 1000 << " Кбит/с" << std::endl;
